...
export function BeneficiariesPage() {
  const pageTickers = uniqueTickers(
    dailyReport.beneficiaries.flatMap((item) => [
        ...
        {dailyReport.beneficiaries.map((item) => (
          <Card key={item.id} title={themeName(item.themeId)} eyebrow="Beneficiary Finder" right={<Badge tone={item.evidenceQuality}>優先 {item.researchPriorityScore}</Badge>}>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <Group title="直接受惠" values={item.directBeneficiaries} groupValues={pageTickers} />
              <Group title="間接受惠" values={item.indirectBeneficiaries} groupValues={pageTickers} />
              <Group title="隱藏受惠" values={item.hiddenBeneficiaries} groupValues={pageTickers} />
              <Group title="可能受傷" values={item.companiesMayBeHurt} groupValues={pageTickers} danger />
            </div>
